/**
 * Static mapping of all 50 U.S. states plus the District of Columbia.
 *
 * Used by /state/[slug] pages, the /states index, and the sitemap.
 * Slugs are lowercase and hyphenated so they can be used directly in URLs
 * without any additional normalization.
 */

export interface StateInfo {
  /** Two-letter USPS code, e.g. "MI". */
  code: string;
  /** Full name, e.g. "Michigan". */
  name: string;
  /** URL slug, e.g. "michigan" or "district-of-columbia". */
  slug: string;
}
...
const BY_SLUG: Map<string, StateInfo> = new Map(
  STATES.map((s) => [s.slug, s])
);
const BY_CODE: Map<string, StateInfo> = new Map(
...
/** All slugs, used by generateStaticParams on /state/[slug]. */
