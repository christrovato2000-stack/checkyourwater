...
export async function GET(request: Request) {
  ...
  if (!token) {
    return page(
      ...
      "This unsubscribe link is missing a token. Please use the link from a CheckYourWater email.",
      400
    );
  }
  ...
  if (data.unsubscribed_at) {
    return page(
      ...
      "You will not receive any further notifications from CheckYourWater.",
      200
    );
  }
  ...
  if (updateError) {
    return page(
      ...
      "We could not unsubscribe you. Please try the link again or contact us if the problem continues.",
      500
    );
  }
  ...
  return page(
    ...
    "You will not receive any further notifications from CheckYourWater. If this was a mistake, you can always sign up again from the homepage.",
    200
  );
}
